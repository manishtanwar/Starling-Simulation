...
{
	if(std::isnan(0.0)) std::cout<<"Sab galat12"<<'\n';
	float dist = (a.x - this->x)*(a.x - this->x);
	dist += (a.y - this->y)*(a.y - this->y);
	dist += (a.z - this->z)*(a.z - this->z);
	// std::cout<<dist<<'\n';
	if(dist < 0) std::cout<<"Sab galat\n";
	// if(std::isnan(dist)) std::cout<<"Sab galat12"<<'\n';
	dist = sqrt(dist);
	std::cout<<std::setprecision(25);
	// if(std::isnan(dist)) std::cout<<"Sab galat1 "<<aasdf<<'\n';
	return dist;
}

float tuple::magnitude()
{
	tuple tmp(0.0,0.0,0.0);
